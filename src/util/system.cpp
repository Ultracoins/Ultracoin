...
const char * const ULTRACOIN_CONF_FILENAME = "ultracoin.conf";
const char * const ULTRACOIN_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/ultracoin/ultracoin/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //ultracoin-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", ULTRACOIN_SETTINGS_FILENAME);
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Ultracoin
    // macOS: ~/Library/Application Support/Ultracoin
    // Unix-like: ~/.ultracoin
#ifdef WIN32
    // Windows
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Ultracoin";
#else
    // Unix-like
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", ULTRACOIN_CONF_FILENAME);
