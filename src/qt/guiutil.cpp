...
#include <qt/ultracoinaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Ultracoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new UltracoinAddressEntryValidator(parent));
    widget->setCheckValidator(new UltracoinAddressCheckValidator(parent));
}

bool parseUltracoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no ultracoin: URI
    if(!uri.isValid() || uri.scheme() != QString("ultracoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!UltracoinUnits::parse(UltracoinUnits::UC, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseUltracoinURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseUltracoinURI(uriInstance, out);
}

QString formatUltracoinURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("ultracoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(UltracoinUnits::format(UltracoinUnits::UC, info.amount, false, UltracoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openUltracoinConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", ULTRACOIN_CONF_FILENAME));
    ...
    /* Open ultracoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Ultracoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Ultracoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Ultracoin (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Ultracoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "ultracoin.desktop";
    return GetAutostartDir() / strprintf("ultracoin-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a ultracoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Ultracoin\n";
        else
            optionFile << strprintf("Name=Ultracoin (%s)\n", chain);
