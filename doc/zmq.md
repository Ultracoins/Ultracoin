...
The Ultracoin Core daemon can be configured to act as a trusted "border
router", implementing the ultracoin wire protocol and relay, making
...
The ZeroMQ feature in Ultracoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building ultracoind:
...
For instance:
    ...
    $ ultracoind -zmqpubhashtx=tcp://127.0.0.1:242563 \
               -zmqpubhashtx=tcp://192.168.1.2:242563 \
               -zmqpubrawtx=ipc:///tmp/ultracoind.tx.raw \
...
These options can also be provided in ultracoin.conf.
...
From the perspective of ultracoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into ultracoind directly. Furthermore, no information is
...
using. Ultracoind appends an up-counting sequence number to each
