...
TEST_ULTRACOIN_BINARY="src/test/test_ultracoin"
...
if [[ ! -e ${TEST_ULTRACOIN_BINARY} ]]; then
    echo "Error: Executable ${TEST_ULTRACOIN_BINARY} not found. Run \"./configure --enable-lcov\" and compile."
    exit 1
fi
...
while [[ ${TEST_RUN_ID} -lt ${N_TEST_RUNS} ]]; do
    ...
    if ! BOOST_TEST_RUN_FILTERS="${BOOST_TEST_RUN_FILTERS}" ${TEST_ULTRACOIN_BINARY} > "${TEST_OUTPUT_TEMPFILE}" 2>&1; then
